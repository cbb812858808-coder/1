...
OUT='data/market.json'; UA='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/126 Safari/537.36'; FIELDS='f12,f13,f14,f2,f3,f5,f6,f8,f9,f10'
...
def eastmoney_host(host):
 s=requests.Session();s.headers.update({'User-Agent':UA,'Accept':'application/json, text/plain, */*','Referer':'https://quote.eastmoney.com/','Connection':'keep-alive'});rows=[];pn=1;pz=100
 while pn<=60:
  q={'pn':pn,'pz':pz,'po':1,'np':1,'ut':'bd1d9ddb04089700cf9c27f6f7426281','fltt':2,'invt':2,'fid':'f3','fs':'m:0+t:6,m:0+t:80,m:1+t:2,m:1+t:23','fields':FIELDS}
  r=s.get(f'https://{host}/api/qt/clist/get',params=q,timeout=12);r.raise_for_status();data=r.json().get('data') or {};diff=data.get('diff') or []
  if not diff:break
...
def fetch():
 errors=[]
 for host in ['82.push2.eastmoney.com','push2.eastmoney.com','56.push2.eastmoney.com']:
...
with open(OUT,'w',encoding='utf8') as f:json.dump({'version':'5.3-public-multisource','generated_at':now.isoformat(),'trade_date':now.date().isoformat(),'source':rows[0]['source'],'stocks':rows},f,ensure_ascii=False)
